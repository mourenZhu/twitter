...
2. 点赞
    1. 点赞推文（推文点赞数+1）`已完成` `已测试`
    2. 查看该推文有哪些用户点赞`未开发（在timeline中开发比较有意义）`
    3. 查看用户点赞的推文id`已完成` `已测试`
    4. 取消点赞`已完成` `已测试`
3. 转发
...
4. 引用
    1. 引用推文（推文引用数+1）`已完成` `已测试`
    2. 查看引用了推文的推文 `(controller层和service层中方法已写，但并为实现)暂不开发，在timeline中获取毕竟有意义`
    3. 删除引用推文（引用后也是一条推文，所以删除后需要删除引用表中的记录）`已完成` `已测试`
